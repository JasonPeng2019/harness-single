...
def run(mode: str, active: bool) -> dict[str, object]:
    ...
    harness_config = HarnessConfig(
        ...
        max_jsonl_tail_bytes=512_000,
        stable_read_retries=4,
        stable_read_delay_seconds=0.01,
    ...
    try:
        ...
        if not request.exists():
            raise RuntimeError(
                "synthetic board-free controller did not publish its request"
            )
        if active:
            manager_log = workspace / "manager-events.jsonl"
            harness_log = workspace / "synthetic-harness-events.jsonl"
            manager_log.write_text(
                '{"event":"heartbeat","actor":"manager"}\n', encoding="utf-8"
            )
            harness_log.write_text(
                '{"event":"observation","actor":"harness"}\n', encoding="utf-8"
            )
            watcher_config = WatcherConfig(
                repository_root=ROOT,
                runtime_root=destination / "harness_watcher",
                observed_log_roots=(
                    manager_log,
                    harness_log,
                    workspace / "synthetic_codex.jsonl",
                ),
                observed_sources=(
        ...
        try:
            ...
            event_exit, events = watch_once(
                harness_config, no_write=False, stream=event_out
            )
        finally:
            event_out.close()
        ...
        after_out = (destination / "harness-after-events.jsonl").open(
            "w", encoding="utf-8"
        )
        try:
            after_exit, after_events = watch_once(
                harness_config, no_write=False, stream=after_out
            )
        finally:
            after_out.close()
        if controller.returncode != 0:
            raise RuntimeError(
                f"synthetic controller failed: {controller.returncode} {stdout} {stderr}"
            )
        result = {
            "mode": mode,
            "active": active,
            "actual_suite_scan_exit": actual_exit,
            "synthetic_watch_exit": event_exit,
            "synthetic_after_exit": after_exit,
            ...
            "terminal_status": json.loads(
                (workspace / "synthetic_controller.status.json").read_text(
                    encoding="utf-8"
                )
            ).get("state"),
            "watcher_poll_alert": getattr(watcher_result.get("alert"), "alert_id", None)
            if active
            else None,
            "watcher_runtime_created": (destination / "harness_watcher").exists(),
            "four_way_log_paths": {
                category: (
                    destination / "harness_watcher" / category / "events.jsonl"
                ).is_file()
                for category in ("orchestrator", "harness", "watcher")
            }
            | {
                "subagent": any(
                    (destination / "harness_watcher" / "subagents").glob(
                        "*/events.jsonl"
                    )
                )
            },
        ...
        if active and integration_runtime.exists():
            shutil.copytree(
                integration_runtime, destination / "harness-integration-runtime"
            )
...
if __name__ == "__main__":
    ...
    (RESULTS / "canary_summary.json").write_text(
        json.dumps(all_results, indent=2) + "\n", encoding="utf-8"
    )
    (RESULTS / "canary_cleanup.json").write_text(
        json.dumps(
            {"root_harness_watcher_residue": (ROOT / "harness_watcher").exists()},
            indent=2,
        )
        + "\n",
        encoding="utf-8",
