...
def config(runtime: str) -> Path:
    ...
    path.write_text(json.dumps({
        ...
        "observed_log_roots": ["harness_watcher_implementation/test_results/lifecycle_canary/empty.log"],
        "poll_interval_seconds": 2,
        "evaluator_command": [sys.executable, "-c", "import json; print(json.dumps({'defect':False,'kind':None,'severity':'warning','summary':'healthy','implicated':[],'evidence':[]}))"],
    }), encoding="utf-8")
    return path


def call(path: Path, command: str, *extra: str) -> tuple[int, dict[str, object]]:
    run = subprocess.run([sys.executable, "-m", "harness_watcher_implementation", "--config", str(path), command, *extra], cwd=ROOT, text=True, capture_output=True, timeout=20)
...
def wait_not_running(path: Path, reason: str | None = None) -> dict[str, object]:
    ...
    while time.monotonic() < deadline:
        ...
        if not state["running"] and (reason is None or state.get("state", {}).get("exit_reason") == reason):
            return state
        time.sleep(.1)
...
def main() -> None:
    ...
    subprocess.run([sys.executable, "-c", wrapper, str(owner_loss)], cwd=ROOT, check=True, timeout=20)
    lost = wait_not_running(owner_loss, "owner-identity-lost")
    result = {
        "start": [start_code, start], "duplicate": [duplicate_code, duplicate],
        "live_status": [status_code, status], "stop": [stop_code, stop], "stopped_status": stopped,
        "owner_loss_status": lost,
        "live_has_pid_and_creation": bool(status.get("state", {}).get("watcher", {}).get("pid")) and isinstance(status.get("state", {}).get("watcher", {}).get("created_utc"), str) and ":" in status.get("state", {}).get("watcher", {}).get("created_utc", ""),
        "cooperative_stop_reason": status.get("state", {}).get("exit_reason") is None and stopped.get("state", {}).get("exit_reason") == "stop-requested",
        "owner_loss_reason": lost.get("state", {}).get("exit_reason") == "owner-identity-lost",
    }
