...
from .attention import AttentionValidationError, append_producer_record, make_source_record
...
def _same(record: object) -> bool:
    if not isinstance(record, dict) or not isinstance(record.get("pid"), int) or not isinstance(record.get("created_utc"), str):
        return False
    actual = _identity(record["pid"])
    return actual is not None and actual == {"pid": record["pid"], "created_utc": record["created_utc"]}
...
def _terminal(service: Path, runtime: Path, state: dict[str, object], reason: str, startup_token: str, watcher: dict[str, object] | None) -> bool:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "record-attention":
        ...
        if (args.role, args.source_id) not in cfg.attention_producers:
            ...
            return 2
        try:
            metadata_text = Path(args.metadata_file).read_text(encoding="utf-8") if args.metadata_file else (args.metadata if args.metadata is not None else "{}")
            metadata = json.loads(metadata_text)
            if not isinstance(metadata, dict):
                raise AttentionValidationError("metadata must be an object")
            record_value = make_source_record(recorder=args.source_id, epoch_id=args.epoch_id, event_id=args.event_id, kind=args.kind, source_timestamp_utc=args.source_timestamp_utc, metadata=metadata)
            path = append_producer_record(runtime, role=args.role, source_id=args.source_id, record=record_value, lock_timeout_seconds=cfg.attention_lock_timeout_seconds)
        except (OSError, json.JSONDecodeError, AttentionValidationError, ValueError, TimeoutError) as exc:
    ...
    if not settings.harness_watcher_active:
        ...
        return 0
    if args.cmd == "poll":
        print(json.dumps({"alert": poll(cfg, TerraHighEvaluator(cfg.evaluator_command) if cfg.evaluator_enabled else None)["alert"]}, default=str))
        return 0
    if args.cmd == "status":
        state = _read(service)
        running = bool(state and _same(state.get("watcher")) and _same(state.get("owner")) and not state.get("stop_requested") and not state.get("exit_reason"))
    ...
    if args.cmd == "start":
        ...
        try:
            ...
            owner = _identity(args.owner_pid if args.owner_pid is not None else os.getppid())
            if owner is None:
                raise RuntimeError("cannot establish owner process identity")
            startup_token = str(uuid.uuid4())
            _atomic(service, {"schema": "harness-watcher-service/v1", "watcher": None, "owner": owner, "startup_token": startup_token, "startup_status": "STARTING", "started_utc": datetime.now(timezone.utc).isoformat(), "stop_requested": False, "exit_reason": None, "evaluator_enabled": cfg.evaluator_enabled})
            command = [sys.executable, "-m", "harness_watcher_implementation"] + (["--config", args.config] if args.config else []) + ["serve", "--startup-token", startup_token]
            flags = 0
            if os.name == "nt":
                flags = getattr(subprocess, "CREATE_NO_WINDOW", 0) | getattr(subprocess, "CREATE_BREAKAWAY_FROM_JOB", 0)
            child = subprocess.Popen(command, cwd=str(cfg.repository_root), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=(os.name != "nt"), creationflags=flags)
            deadline = time.monotonic() + 5
            while time.monotonic() < deadline:
                published = _read(service)
                observed_watcher = published.get("watcher") if isinstance(published, dict) else None
                if isinstance(published, dict) and published.get("startup_token") == startup_token and published.get("startup_status") == "READY" and isinstance(observed_watcher, dict) and isinstance(observed_watcher.get("pid"), int) and _same(observed_watcher):
                    print(json.dumps({"started": True, "pid": observed_watcher["pid"], "owner": owner}))
                    return 0
                if child.poll() is not None:
                    break
                time.sleep(.05)
            ...
            log(runtime, "watcher", "SERVICE_ERROR", {"phase": "start", "error": str(exc)[:500]})
    ...
    if args.cmd == "serve":
        ...
        try:
            ...
            if state.get("startup_token") != args.startup_token:
                raise RuntimeError("startup token does not match published service state")
            ...
            log(runtime, "watcher", "SERVICE_STARTED", {"pid": os.getpid(), "evaluator_enabled": cfg.evaluator_enabled})
            while True:
                if not _same(owner):
                    reason = "owner-identity-lost"
                    break
                poll(cfg, TerraHighEvaluator(cfg.evaluator_command) if cfg.evaluator_enabled else None)
