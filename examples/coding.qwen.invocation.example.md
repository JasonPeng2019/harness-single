...
The provider adapter owns the native command shape: `qwen exec`,
...
```powershell
python -m orchestrator_harness --config <config> adapter install --host qwen --project-root <project>
```
...
only and does not acknowledge manager queue work.  This repository proves the
route deterministically; it makes no live Qwen hook-session claim.
