...
`python -m firmware_acceptance.c3_harness` is serve-only. Before readiness it
validates the C1 lock, delegated authorization, topology release, candidate
inputs, five-file read-only seed, manifest, policy, and templates. Its only
target-worker ingress is the fixed token-bound P1 inbox; it does not accept a
worker-supplied path, command, model, environment, endpoint, or hardware handle.
Each assignment gets a candidate-created Git worktree and captured lane-controller
child. The harness retains immutable admission, completion, and session-operation
evidence, and shutdown remains blocked while a child or operation is active.

Server-limitation requests intentionally remain locked at this layer until the
separate `LimitationEvidenceAdapter` has supplied exact candidate-owned adapter
references. This facade never repins, repairs, bypasses, or fabricates physical
certification.
