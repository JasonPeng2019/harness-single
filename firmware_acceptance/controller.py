...
class FirmwareAcceptanceController:
    ...
    def session_derive_authorization(self, proposal_path: Path, decision_path: Path, authorization_path: Path, verifier: SignatureVerifier) -> dict[str, Any]:
        ...
        decision, decision_sha = self._read_external_bound(decision_path, _DECISION_KEYS)
    ...
    @staticmethod
    def _validate_continuation_response(continuation: dict[str, Any], response: Any) -> bool:
        if not isinstance(response, dict):
            return False
        if continuation.get("kind") == "choice":
    ...
    def _read_external_bound(self, path: Path, keys: set[str]) -> tuple[dict[str, Any], str]:
        value = self._load_external(path, keys)
        return value, hashlib.sha256(path.resolve().read_bytes()).hexdigest()
