...
def _write_policy(
    root: Path, *, launchers: object = POLICY_JSON, exclusions: str = ""
) -> None:
...
class BoundedPolicyModuleTests(unittest.TestCase):
    ...
    def test_launcher_load_rejects_incomplete_or_wrong_shapes(self) -> None:
        invalid = (
            {
                "schema": "bounded-launchers/v2",
                "launcher_categories": POLICY_JSON["launcher_categories"],
            },
    ...
    def test_gitignore_file_directory_wildcard_negation_and_sibling_prefix(
        self,
    ) -> None:
        _write_policy(
            self.root,
        ...
        self.assertEqual(
            {},
            self._guard(
                'powershell -File "tools/lane launchers/start.ps1"', cwd=self.root
            ),
        )
        self.assertTrue(
            _deny(self._guard("python nested/lane-launchers/bounded.py", cwd=self.root))
        )
        self.assertTrue(
            _deny(
                self._guard(
                    "python tools/lane-launchers-backup/start.py", cwd=self.root
                )
            )
        )
        self.assertFalse(
            is_excluded(
                ...
                exclusions=validate_exclusions(self.root),
            )
        )
    ...
    def test_quoted_paths_workdir_precedence_and_outside_root_paths(self) -> None:
        ...
        outside = (
            self.root.parent / f"{self.root.name}-outside" / "launchers" / "start.py"
        )
    ...
    def test_command_segments_honor_powershell_quote_and_backtick_rules(self) -> None:
        ...
        self.assertEqual(
            ["python -c 'print(1); print(2)'", "python task.py"],
            command_segments(quoted),
        )
        double_quoted = 'python -c "print(1) | print(2)" | python task.py'
        self.assertEqual(
            ['python -c "print(1) | print(2)"', "python task.py"],
            command_segments(double_quoted),
        )
        ...
        self.assertEqual(
            ["python task.py `; python other.py"], command_segments(backtick)
        )
        single_quoted_backslash = r"python 'a\;b'; python task.py"
        self.assertEqual(
            [r"python 'a\;b'", "python task.py"],
            command_segments(single_quoted_backslash),
        )
        double_quote_backtick_escape = 'python "a`"b"; python task.py'
        self.assertEqual(
            ['python "a`"b"', "python task.py"],
            command_segments(double_quote_backtick_escape),
        )

    def test_supervisor_exclusion_does_not_exempt_appended_covered_segment(
        self,
    ) -> None:
    ...
    def test_current_packaged_policy_excludes_only_approved_orchestration_paths(
        self,
    ) -> None:
        packaged = {
            path.as_posix(): data for path, data in packaged_codex_assets().items()
        }
        ...
        exclusions = packaged[".codex/policies/bounded-exclusions.gitignore"].decode(
            "utf-8"
        )
        lines = [
            line.strip()
    ...
    def test_fail_closed_missing_malformed_non_utf8_and_incomplete_policy(self) -> None:
        ...
        (self.root / ".codex" / "policies" / "bounded-launchers.json").write_text(
        ...
        _write_policy(self.root, exclusions="task.py\n")
        (
            self.root / ".codex" / "policies" / "bounded-exclusions.gitignore"
        ...
        self.assertIn(
            "configuration error",
            incomplete["hookSpecificOutput"]["permissionDecisionReason"],
        )
    ...
    def test_fail_closed_when_git_cannot_start_or_evaluate(self) -> None:
        ...
        with mock.patch(
            "orchestrator_harness.codex_bounded_policy.subprocess.run",
    ...
    def test_git_evaluation_requires_a_repository(self) -> None:
        ...
        shutil.copy2(
            self.root / ".codex" / "policies" / "bounded-launchers.json", policies
        )
        (policies / "bounded-exclusions.gitignore").write_text(
            "task.py\n", encoding="utf-8"
        )
    ...
    def test_stateless_concurrent_evaluation(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=8) as pool:
            results = list(
                pool.map(
                    lambda _: guard_pre_tool_use(payload, policy_root=self.root),
                    range(40),
                )
            )
    ...
    def test_missing_command_field_is_a_noop(self) -> None:
        self.assertEqual(
            {}, guard_pre_tool_use({"tool_input": {}}, policy_root=self.root)
        )
    ...
    def test_lane_managed_stable_runner_exclusion_has_no_session_deadline(self) -> None:
        ...
        self.assertTrue(
            _deny(self._guard("python .codex/scripts/verify.py", cwd=self.root))
        )
...
class InstalledHookTests(unittest.TestCase):
    ...
    def test_installed_hook_denies_nested_covered_commands_and_allows_exclusions(
        self,
    ) -> None:
        install = install_codex_adapter(self.project)
        self.assertTrue(install["current"])
        self.assertTrue(
            (self.project / ".codex" / "policies" / "bounded-launchers.json").is_file()
        )
        ...
        allowed = self._hook_result(
            {
                "cwd": str(self.project),
                "tool_input": {
                    "command": "python -I .codex/scripts/stable_runner.py invocation.json"
                },
            }
        )
        ...
        denied_appended = self._hook_result(
            {
                "cwd": str(self.project),
                "tool_input": {
                    "command": "python -I .codex/scripts/stable_runner.py invocation.json; python .codex/scripts/verify.py"
                },
            }
        )
        self.assertEqual(
            "deny", denied_appended["hookSpecificOutput"]["permissionDecision"]
        )
