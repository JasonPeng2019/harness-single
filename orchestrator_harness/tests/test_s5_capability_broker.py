...
class _Claims:
    def __init__(
        ...
        events: list[str],
        *,
        owner: dict[str, Any] = OWNER,
        arm_failures: list[str] | None = None,
        release_failures: list[str] | None = None,
        retention_failures: list[str] | None = None,
    ) -> None:
        self.root = root
        self.events = events
...
class _ContendedClaims:
    def __init__(
        self, shared: _SharedClaims, root: Path, owner: dict[str, Any] = OWNER
    ) -> None:
        self.shared = shared
        self.root = root
        self.owner = dict(owner)
...
def _campaign_pack() -> FirmwareCampaignPack:
    return FirmwareCampaignPack(
        ...
        policy={
            "policy": "caller-declared-policy-v1",
            "revision": "caller-supplied-revision",
        },
    )


def _firmware_snapshot(
    request: CapabilityRequest, pack: FirmwareCampaignPack
) -> CapabilitySnapshot:
...
def _firmware_adapter(
    ...
    *,
    ...
    pack=None,
) -> tuple[FirmwareHardwareAdapter, FirmwareCampaignPack, Any]:
    ...
    adapter = FirmwareHardwareAdapter(
        ...
        identity_provider=lambda pid: {
            "pid": pid,
            "created_utc": "fake-child",
            "creation_identity": "fake-exact",
        },
...
def _firmware_request_and_approval(
    adapter: FirmwareHardwareAdapter,
    pack: FirmwareCampaignPack,
    *,
    request_id: str = "hardware-request",
    resource: str = "board:stm-a",
    expiry: float = 100.0,
    action: str = "observe",
...
def _approval_for_native_claim(
    request: CapabilityRequest, adapter: FakeCapabilityAdapter
) -> dict[str, Any]:
...
def _broker(
    ...
    *,
    ...
    return CapabilityBroker(
        ...
        evidence_sink=(
            lambda record: (
                events.append(f"published:{record['state']}")
...
class S5CapabilityBrokerTests(unittest.TestCase):
    def _approval_for(
        self, request_value: dict[str, Any], adapter: FakeCapabilityAdapter
    ) -> dict[str, Any]:
    ...
    def test_S5_F1_generic_success_binds_exact_facts_and_releases_after_cleanup(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            ...
            request = _request()
            result = _broker(adapter, claims, events=events).execute(
                request, self._approval_for(request, adapter)
            )
            ...
            self.assertEqual(
                result.record["raw_result_sha256"],
                hashlib.sha256(
                    b'{"request_id":"request-1","status":"completed"}'
                ).hexdigest(),
            )
    ...
    def test_S5_F2_closed_request_rejects_endpoint_material_before_observation(
        self,
    ) -> None:
        adapter = FakeCapabilityAdapter({"synthetic": ("read",)})
        value = _request()
    ...
    def test_S5_F5_dispatch_failure_is_terminal_failure_and_cleanup_precedes_release(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            events: list[str] = []
            ...
            claims = _Claims(Path(temporary), events)
            request = _request()
            result = _broker(adapter, claims).execute(
                request, self._approval_for(request, adapter)
            )
    ...
    def test_S5_F5_unproved_cleanup_retains_exact_claim_and_is_uncertain(self) -> None:
        with TemporaryDirectory() as temporary:
            adapter = FakeCapabilityAdapter(
                {"synthetic": ("read",)}, cleanup_proved=False
            )
            claims = _Claims(Path(temporary), [])
            request = _request()
            result = _broker(adapter, claims).execute(
    ...
    def test_S5_F3_same_resource_serializes_and_disjoint_resource_progresses(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            ...
            def make_broker() -> CapabilityBroker:
                return CapabilityBroker(
                    ...
                    policy_verifier=lambda *_: True,
                    identity_provider=lambda: dict(OWNER),
                    claims_factory=lambda lane, request_id: _ContendedClaims(
            ...
            second_same = _request(
                request_id="request-2", resources=["resource-a"], lane_id="lane-b"
            )
            ...
            thread_one = threading.Thread(
                target=lambda: first_result.setdefault(
                    "value", first_broker.execute(first, first_approval)
                ),
                daemon=True,
            )
            thread_one.start()
            ...
            thread_two = threading.Thread(
                target=lambda: second_result.setdefault(
                    "value", second_broker.execute(second_same, second_approval)
                ),
                daemon=True,
            )
            thread_two.start()
            ...
            disjoint = _request(
                request_id="request-4", resources=["resource-b"], lane_id="lane-b"
            )
            first_result.clear()
            second_result.clear()

            def make_broker_for(
                adapter_value: FakeCapabilityAdapter,
            ) -> CapabilityBroker:
                return CapabilityBroker(
                    adapter_value,
                    approval_verifier=_Verifier(),
            ...
            thread_one = threading.Thread(
                target=lambda: first_result.setdefault(
                    "value",
                    first_broker.execute(first, self._approval_for(first, adapter)),
                ),
                daemon=True,
            )
            thread_one.start()
            ...
            thread_two = threading.Thread(
                target=lambda: second_result.setdefault(
                    "value",
                    second_broker.execute(
                        disjoint, self._approval_for(disjoint, adapter)
                    ),
                ),
                daemon=True,
            )
    ...
    def test_S5_F4_uncertain_resource_evidence_denies_without_dispatch(self) -> None:
                ...
                on_wait(
                    {
                        ...
                        "reason": "fake incomplete inventory",
                    }
                )
        ...
        adapter = FakeCapabilityAdapter({"synthetic": ("read",)})
        request = _request()
        result = _broker(adapter, UncertainClaims()).execute(
            request, self._approval_for(request, adapter)
        )
        self.assertEqual("DENIED", result.outcome)
    ...
    def test_S5_F4_broker_consumes_native_resource_claim_owner_and_releases_it(
        self,
    ) -> None:
    ...
    def test_S5_F6_legacy_and_coding_records_are_not_mixed_into_generic_route(
        self,
    ) -> None:
        adapter = FakeCapabilityAdapter({"synthetic": ("read",)})
        claims = _Claims(Path("."), [])
    ...
    def test_S5_F8_exact_retry_reuses_terminal_result_and_changed_replay_denies(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            adapter = FakeCapabilityAdapter({"synthetic": ("read",)})
    ...
    def test_S5_R1_001_native_claim_is_armed_before_dispatch_and_retained_after_uncertainty(
        self,
    ) -> None:
        class ArmObservingAdapter(FakeCapabilityAdapter):
            ...
            def dispatch(self, permit):
                claim = json.loads(
                    (self.claim_root / claim_filename("resource-a")).read_text(
                        encoding="utf-8"
                    )
                )
        ...
        with TemporaryDirectory() as temporary:
            ...
            retained = json.loads(
                (root / "claims" / claim_filename("resource-a")).read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_S5_R1_003_default_boundary_tracks_descendant_and_proves_empty_cleanup(
        self,
    ) -> None:
    ...
    def test_S5_R1_003_boundary_contradiction_retains_claim_even_when_adapter_says_proved(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            events: list[str] = []
            ...
            self.assertFalse(result.record["claims_released"])
            self.assertIn(
                "cleanup boundary reports live_members",
                result.record["cleanup_validation"]["reasons"],
            )
            self.assertNotIn("release", events)

    def test_S5_R1_004_fresh_broker_reuses_durable_terminal_and_rejects_replays(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            first = _broker(adapter, first_claims, state_root=state_root).execute(
                request, approval
            )
            self.assertEqual("PASS", first.outcome)
            second_claims = _Claims(root / "claims-2", [])
            second = _broker(adapter, second_claims, state_root=state_root).execute(
                copy.deepcopy(request), copy.deepcopy(approval)
            )
            self.assertEqual(first.to_record(), second.to_record())
            self.assertEqual(1, adapter.dispatch_calls)
            ...
            changed = _broker(
                adapter, _Claims(root / "claims-3", []), state_root=state_root
            ).execute(changed_request, approval)
            ...
            approval_replay = _broker(
                adapter, _Claims(root / "claims-4", []), state_root=state_root
            ).execute(request, changed_approval)
            self.assertEqual("DENIED", approval_replay.outcome)
            self.assertEqual(
                "APPROVAL_REPLAY", approval_replay.record["denial"]["reason_code"]
            )
            self.assertEqual(1, adapter.dispatch_calls)

    def test_S5_R1_005_cleanup_contradictions_and_retention_failures_never_release(
        self,
    ) -> None:
        variants = (
        ...
        for index, boundary in enumerate(variants):
            with self.subTest(index=index), TemporaryDirectory() as temporary:
                ...
                claims = _Claims(
                    Path(temporary),
                    events,
                    retention_failures=["retention-proof-failed"],
                )
                ...
                self.assertFalse(result.record["claims_released"])
                self.assertEqual(
                    ("retention-proof-failed",), result.record["retention_failures"]
                )
                self.assertNotIn("release", events)
    ...
    def test_S5_R1_006_recursive_public_authority_aliases_are_rejected(self) -> None:
        ...
        for alias in aliases:
            with self.subTest(alias=alias), TemporaryDirectory() as temporary:
                ...
                result = _broker(adapter, _Claims(Path(temporary), [])).execute(
                    request, {}
                )
                self.assertEqual("DENIED", result.outcome)
                self.assertEqual(0, adapter.support_calls)
                self.assertNotIn(
                    alias, json.dumps(result.to_record(), sort_keys=True).lower()
                )
        ...
        for alias in aliases:
            with self.subTest(snapshot_fact=alias), TemporaryDirectory() as temporary:
                ...
                self.assertEqual("DENIED", result.outcome)
                self.assertEqual(
                    "SNAPSHOT_INVALID", result.record["denial"]["reason_code"]
                )
            ...
            with self.subTest(public_fact=alias):
                with self.assertRaises(CapabilityError):
                    AdapterResult(
                        ...
                        {"status": "PASS"},
                        {"adapter_id": "fake", "adapter_version": "v1"},
                    )
                with self.assertRaises(CapabilityError):
                    AdapterResult(
                        True,
        ...
        self.assertTrue(
            _Verifier().verify(
                public_approval.signed_payload,
                public_approval.signature,
                public_approval.public_key,
            )
        )
        self.assertEqual(
            public_approval.to_record(),
            json.loads(public_approval.signed_payload.decode("utf-8"))
            | {"signature": public_approval.signature},
        )
        self.assertEqual(
            canonical_json_bytes(
                {
    ...
    def test_S5_R1_009_ordinary_preclaim_failures_are_typed_published_denials(
        self,
    ) -> None:
        ...
        cases.append(
            (
                "supports",
                SupportsRaises({"synthetic": ("read",)}),
                lambda request, snapshot, approval: True,
            )
        )
        cases.append(
            (
                "observe",
                ObserveRaises({"synthetic": ("read",)}),
                lambda request, snapshot, approval: True,
            )
        )
        ...
        result = CapabilityBroker(
            adapter,
            approval_verifier=lambda *_: (_ for _ in ()).throw(
                RuntimeError("signature exploded")
            ),
            policy_verifier=lambda *_: True,
            identity_provider=lambda: dict(OWNER),
        ...
        self.assertEqual("DENIED", result.outcome)
        self.assertEqual(
            "APPROVAL_SIGNATURE_INVALID", result.record["denial"]["reason_code"]
        )
        self.assertEqual(0, result.record["dispatch_count"])
        self.assertTrue(published)
        ...
        policy_result = CapabilityBroker(
            policy_adapter,
            approval_verifier=_Verifier(),
            policy_verifier=lambda *_: (_ for _ in ()).throw(
        ...
        self.assertEqual(
            "APPROVAL_POLICY_UNAVAILABLE", policy_result.record["denial"]["reason_code"]
        )
        ...
        parse_result = _broker(adapter, _Claims(Path("."), [])).execute(
            _request(request_id="approval-parse"), malformed
        )
        self.assertEqual("DENIED", parse_result.outcome)
        self.assertEqual(
            "ADMISSION_FAILED", parse_result.record["denial"]["reason_code"]
        )
        self.assertEqual(0, parse_result.record["dispatch_count"])

    def test_S5_R1_010_malicious_adapter_cannot_mutate_nested_permit_or_hash(
        self,
    ) -> None:
        class MutatingAdapter(FakeCapabilityAdapter):
            ...
            def dispatch(self, permit):
                ...
                self.mutation_blocked = (
                    self.mutation_blocked
                    and permit.request.arguments["value"] == "safe"
                )
                return super().dispatch(permit)

        adapter = MutatingAdapter()
        request = _request()
        result = _broker(adapter, _Claims(Path("."), [])).execute(
            request, self._approval_for(request, adapter)
        )
        self.assertEqual("PASS", result.outcome)
    ...
    def test_S5_R1_003_default_adapter_boundary_releases_after_disposable_process_cleanup(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            try:
                adapter = FirmwareHardwareAdapter(
                    ...
                    transport_factory=lambda process, remaining, request_id, config: (
                        _Transport()
                    ),
                    mcp_protocol_version="2025-03-26",
                    clock=lambda: 10.0,
                ...
                self.skipTest(f"native process boundary unavailable: {exc}")
            request, approval = _firmware_request_and_approval(
                adapter, pack, request_id="default-adapter-boundary"
            )
            claims = _Claims(root, [])
            result = CapabilityBroker(
                adapter,
            ...
            self.assertTrue(result.record["claims_released"])
            self.assertNotIn(
                "session_id", json.dumps(result.to_record(), sort_keys=True)
            )

    def test_S5_R1_004_adapter_cleanup_cache_is_keyed_by_request_and_permit_digest(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            adapter, pack, _ = _firmware_adapter(Path(temporary))
            request_value, approval_value = _firmware_request_and_approval(
                adapter, pack, request_id="cache-request"
            )
            request = CapabilityRequest.from_record(request_value, now_monotonic=0.0)
            snapshot = adapter.observe(request)
            approval = CapabilityApproval.from_record(approval_value)
            permit_a = CapabilityPermit(
                request,
                snapshot,
                approval,
                (),
                dict(OWNER),
                dict(adapter.adapter_identity),
                40.0,
                "a" * 64,
            )
            permit_b = CapabilityPermit(
                request,
                snapshot,
                approval,
                (),
    ...
    def test_S5_R1_008_expiry_is_checked_before_enqueue_and_at_writer_effect(
        self,
    ) -> None:
        for mode in ("before-enqueue", "dispatch"):
            with self.subTest(mode=mode), TemporaryDirectory() as temporary:
                clock = _MutableClock()
                transport = _ExpiryTransport(
                    clock, expire_on="dispatch" if mode == "dispatch" else None
                )
                ...
                adapter, pack, _ = _firmware_adapter(
                    Path(temporary),
                    clock,
                    transport=transport,
                ...
                request, approval = _firmware_request_and_approval(
                    adapter, pack, request_id=f"expiry-{mode}"
                )
                claims = _Claims(Path(temporary), [])
                result = CapabilityBroker(
                    adapter,
                ...
                self.assertFalse(
                    any(
                        message.get("params", {}).get("name") == "get_board_info"
                        for message in transport.messages
                    )
                )
                self.assertTrue(result.record["claims_released"])

    def test_S5_R2_001_caller_declared_pack_maps_actions_and_keeps_lane_and_resource_independent(
        self,
    ) -> None:
        pack = _campaign_pack()
        ...
        for lane_id in ("lane-a", "lane-b"):
            request = CapabilityRequest.from_record(
                _request(
                    request_id=f"pack-{lane_id}",
                    lane_id=lane_id,
                    capability="firmware-acceptance",
                    action="observe",
                    arguments={"board_id": "stm-a"},
        ...
        extra = CapabilityRequest.from_record(
            _request(
                request_id="pack-extra",
                lane_id="lane-a",
                capability="firmware-acceptance",
                action="observe",
                arguments={"board_id": "stm-a", "extra": True},
                resources=["board:stm-a"],
            ),
            now_monotonic=0.0,
        )
    ...
    def test_S5_R2_002_exact_mcp_traffic_policy_duration_binding_and_private_config_stays_private(
        self,
    ) -> None:
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            request, approval = _firmware_request_and_approval(
                adapter, pack, request_id="mcp-traffic"
            )
            claims = _Claims(root, [])
            result = CapabilityBroker(
                adapter,
            ...
            self.assertEqual(
                50.0, adapter.effective_expiry(parsed, snapshot, approval_object, 10.0)
            )
            operation = pack.resolve(parsed)
            self.assertEqual(40, operation.maximum_duration_seconds)
            self.assertEqual("caller-supplied-revision", operation.policy["revision"])

    def test_S5_R2_003_supervised_cleanup_releases_claim_only_after_boundary_and_transport_closed(
        self,
    ) -> None:
        for boundary_complete, transport_closed, expected in (
            ...
            with (
                ...
                if not transport_closed:
                    transport.close_and_join = lambda: (
                        False,
                        {"helpers_stopped": False, "errors": ["transport helper"]},
                    )
                adapter = FirmwareHardwareAdapter(
                    campaign_pack=pack,
                    snapshot_provider=lambda request: _firmware_snapshot(request, pack),
                ...
                request, approval = _firmware_request_and_approval(
                    adapter,
                    pack,
                    request_id=f"cleanup-{boundary_complete}-{transport_closed}",
                )
    ...
    def test_S5_R2_004_zero_argument_declaration_accepts_only_empty_argument_map(
        self,
    ) -> None:
        pack = FirmwareCampaignPack(
            capability="firmware-acceptance",
        ...
        operation = pack.resolve(
            CapabilityRequest.from_record(empty_value, now_monotonic=0.0)
        )
        ...
        with self.assertRaises(CapabilityAdapterUnavailable):
            pack.resolve(
                CapabilityRequest.from_record(nonempty_value, now_monotonic=0.0)
            )
