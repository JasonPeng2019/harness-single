...
class LaneControllerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.policy.parent / "AUTONOMOUS_EXECUTION_POLICY.sha256").write_text(
            self.policy_sha + "  AUTONOMOUS_EXECUTION_POLICY.md\n", encoding="utf-8"
        )
    ...
    def invocation(
        self, *, action: str = "start", label: str = "fake", thread: str | None = None
    ) -> Path:
        paths = {
        ...
        raw = {
            ...
            "task": "T00",
            "phase": "host-only",
            "declared_lane_id": "T00:Fake:one",
            "leases": [],
            "server_snapshot": {"head": "fake"},
            "board_tokens": ["board:fake"],
            ...
            "config_overrides": ['mcp_servers.fake.command="nothing"'],
            "resume_thread_id": thread,
            "output_paths": paths,
            "lane_event_log": str(self.events),
        }
    ...
    def status(self, label: str = "fake") -> dict:
        return json.loads(
            (self.workspace / f"{label}_controller.status.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_start_streams_stdin_records_flags_and_is_discoverable(self) -> None:
        ...
        config_path.write_text(
            json.dumps({"suite_root": str(self.root), "run_globs": ["runs/*"]}),
            encoding="utf-8",
        )
    ...
    def test_resume_without_thread_and_bad_paths_are_rejected(self) -> None:
        self.assertEqual(
            2, controller.main([str(self.invocation(action="resume", label="new"))])
        )
    ...
    def test_schema_less_firmware_rejects_coding_model_settings_aliases(self) -> None:
        ...
        self.assertEqual(
            set(coding_model_settings_only),
            controller._CODING_ONLY_MODEL_SETTINGS_FIELDS,
        )
    ...
    def test_unbound_changed_and_policy_mismatch_prompts_are_rejected(self) -> None:
        ...
        self.prompt.write_text(
            self.prompt.read_text(encoding="utf-8") + "changed", encoding="utf-8"
        )
