"""RED specification for the Qwen Code worker hook wrappers.

The super-cache worker payload declares two Qwen Code hooks (PostToolUse
and Stop) in workspace.settings.hooks (adapters/qwen-code/super-cache/
.qwen/settings.json) that must be implemented by thin wrapper scripts under
adapters/qwen-code/super-cache/.qwen/hooks/.  The payload is copied
unchanged into an arbitrary worker worktree, so each wrapper resolves its
agent workspace relative to its installed location, asks the shared
hook-dispatch.py for a decision, and translates that decision into the
Qwen Code hook output contract.  This module is the test-first RED slice:
the unittest run must fail until the wrapper files exist.
"""
...
SUPER_CACHE_DIR = WORKTREE_ROOT / "adapters" / "qwen-code" / "super-cache"
HOOKS_DIR = SUPER_CACHE_DIR / ".qwen" / "hooks"
SETTINGS_JSON_PATH = SUPER_CACHE_DIR / ".qwen" / "settings.json"
HOOKS_JSON_PATH = SUPER_CACHE_DIR / ".qwen" / "hooks.json"
...
def _load_post_tool_use_wrapper():
    return _load_wrapper(POST_TOOL_USE_WRAPPER_PATH, "qwen_worker_post_tool_use")


def _load_stop_wrapper():
    return _load_wrapper(STOP_WRAPPER_PATH, "qwen_worker_stop")
...
def _declared_commands():
    return [
        hook["command"]
        for event_hooks in _declaration()["hooks"].values()
...
class QwenWorkerHooksTestCase(unittest.TestCase):
    ...
    def test_declaration_wires_exact_wrapper_scripts(self):
        ...
        self.assertIn(
            "python .qwen/hooks/orchestrator_harness_post_tool_use.py", commands
        )
        self.assertIn("python .qwen/hooks/orchestrator_harness_stop.py", commands)

    def test_post_tool_use_matcher_is_any(self):
        post_tool_use = _declaration()["hooks"]["PostToolUse"]
        self.assertEqual(post_tool_use[0]["matcher"], ".*")
    ...
    def test_legacy_hooks_json_is_absent(self):
        self.assertFalse(
            HOOKS_JSON_PATH.is_file(),
            "GREEN must delete the superseded %s" % HOOKS_JSON_PATH,
        )

    def test_settings_preserves_existing_fields(self):
        declaration = _declaration()
        self.assertEqual(declaration["model"], "qwen3-coder")
        self.assertEqual(declaration["approval_mode"], "yolo")
        self.assertEqual(declaration["output_format"], "stream-json")
    ...
    def test_post_tool_use_dispatch_nonzero_return_fails_closed(self):
        wrapper = _load_post_tool_use_wrapper()
        with mock.patch("subprocess.run") as run:
    ...
    def test_stop_dispatch_nonzero_return_fails_closed(self):
        wrapper = _load_stop_wrapper()
        with mock.patch("subprocess.run") as run:
            run.return_value = mock.Mock(stdout="", returncode=1)
            decision = wrapper.dispatch_decision()
    ...
    def test_post_tool_use_main_reads_stdin_and_emits_one_json_object(self):
        wrapper = _load_post_tool_use_wrapper()
        with mock.patch("subprocess.run") as run:
            run.return_value = mock.Mock(
                stdout=json.dumps({"decision": "ALLOW"}), returncode=0
    ...
    def test_stop_main_reads_stdin_and_emits_one_json_object(self):
        wrapper = _load_stop_wrapper()
        with mock.patch("subprocess.run") as run:
            run.return_value = mock.Mock(
                stdout=json.dumps({"decision": "ALLOW"}), returncode=0
