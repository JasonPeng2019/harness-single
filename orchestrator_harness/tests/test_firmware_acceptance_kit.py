...
class FirmwareAcceptanceKitTests(unittest.TestCase):
    ...
    def _limitation_evidence(self, root: Path, *, limitation_id: str = "L-1") -> tuple[AcceptanceBroker, Path, dict[str, object]]:
        ...
        invocation = {"schema":"orchestrator-coding-invocation/v1", "action":"start", "runtime_root":str(root / "runtime"), "resource_lock_root":str(root / "runtime" / "locks"), "run_root":str(repo.root), "repository":repo.declaration(), "prompt_path":str(prompt), "prompt_sha256":hashlib.sha256(prompt.read_bytes()).hexdigest(), "output_paths":{"status":str(status_path),"jsonl":str(workspace / "worker.jsonl"),"stderr":str(workspace / "worker.log"),"last_message":str(workspace / "worker.last")}, "event_log_path":str(root / "runtime" / "LANE_EVENTS.jsonl"), "lane_id":"F.C3.A1", "worker_invocation_id":worker, "task":"causal diagnostic", "phase":"test", "exclusive_resources":[], "codex":{"command":["codex"],"model":"gpt-5.6-terra","reasoning_effort":"medium","service_tier":"priority","sandbox":"danger-full-access","approval_policy":"never","config_overrides":[]}}
        (root / "runtime").mkdir(); invocation_ref = put(invocation_path, invocation)
        result = {"schema":"orchestrator-lane-result/v1","lane_id":"F.C3.A1","worker_invocation_id":worker,"branch":repo.branch,"commit":repo.head,"outcome":"PASS","summary":"bounded evidence only","checks":[{"name":"firmware-limitation-credit","command":"pending","outcome":"PASS"}]}
        result_ref = put(result_path, result)
        status = {"schema":"orchestrator-lane-controller/v1","invocation_schema":"orchestrator-coding-invocation/v1","state":"CODEX_EXITED","exit_code":0,"declared_lane_id":"F.C3.A1","worker_invocation_id":worker,"held_resource_claims":[],"result_valid":True,"result_validation":{"state":"VALID","path":str(result_path),"sha256":result_ref["sha256"],"commit":repo.head},"controller_pid":1,"controller_created_utc":"2026-01-01T00:00:00Z","repository":repo.declaration(),"worktree_root":str(repo.root)}
        ...
        status_path, result_path = workspace / "controller.status.json", workspace / "RESULT.json"
        invocation["run_root"] = str(repo.root); invocation["repository"] = repo.declaration(); invocation["prompt_path"] = str(prompt); invocation["prompt_sha256"] = hashlib.sha256(prompt.read_bytes()).hexdigest(); invocation["output_paths"] = {"status":str(status_path),"jsonl":str(workspace / "worker.jsonl"),"stderr":str(workspace / "worker.log"),"last_message":str(workspace / "worker.last")}; invocation["lane_id"] = "F.C3.P1"
        ...
        result = {**result,"lane_id":"F.C3.P1","branch":repo.branch,"commit":repo.head,"checks":[{"name":"firmware-limitation-credit","command":assignment_ref["sha256"],"outcome":"PASS"}]}; result_ref = put(result_path, result)
        status["declared_lane_id"] = "F.C3.P1"; status["repository"] = repo.declaration(); status["worktree_root"] = str(repo.root); status["result_validation"] = {"state":"VALID","path":str(result_path),"sha256":result_ref["sha256"],"commit":repo.head}; status_ref = put(status_path, status)
