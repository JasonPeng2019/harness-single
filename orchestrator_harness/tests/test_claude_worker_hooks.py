"""RED specification for the Claude Code worker hook wrappers.

The super-cache worker payload declares two Claude Code hooks (PostToolUse
and Stop) that must be implemented by thin wrapper scripts under
adapters/claude-code/super-cache/.claude/hooks/.  The payload is copied
...
Claude Code hook output contract.  This module is the test-first RED slice:
...
SUPER_CACHE_DIR = WORKTREE_ROOT / "adapters" / "claude-code" / "super-cache"
HOOKS_DIR = SUPER_CACHE_DIR / ".claude" / "hooks"
SETTINGS_JSON_PATH = SUPER_CACHE_DIR / ".claude" / "settings.json"
...
def _load_post_tool_use_wrapper():
    return _load_wrapper(POST_TOOL_USE_WRAPPER_PATH, "claude_worker_post_tool_use")


def _load_stop_wrapper():
    return _load_wrapper(STOP_WRAPPER_PATH, "claude_worker_stop")


def _declared_commands():
    declaration = json.loads(SETTINGS_JSON_PATH.read_text(encoding="utf-8"))
    return [
        hook["command"]
...
class ClaudeWorkerHooksTestCase(unittest.TestCase):
    ...
    def test_declaration_wires_exact_wrapper_scripts(self):
        ...
        self.assertIn(
            "python .claude/hooks/orchestrator_harness_post_tool_use.py", commands
        )
        self.assertIn("python .claude/hooks/orchestrator_harness_stop.py", commands)
