...
class S3OperatorJourneyTests(unittest.TestCase):
    ...
    def _dual_route_statuses(self) -> tuple[Path, Path]:
        coding = self.fixture.status(
            run="coding", label="coding", doer="Coding", task="C01",
        ...
        firmware = self.fixture.status(
            run="firmware", label="firmware", doer="Firmware", task="F01",
            controller_pid=201, codex_pid=202, board_tokens=["STM-A"],
    ...
    def test_S3_DUAL_001_legacy_example_is_schema_less_and_coding_v1_is_separate(self) -> None:
        legacy = json.loads((REPOSITORY_ROOT / "examples" / "legacy-firmware.invocation.example.json").read_text(encoding="utf-8"))
        coding = json.loads((REPOSITORY_ROOT / "examples" / "coding.invocation.example.json").read_text(encoding="utf-8"))
    ...
    def test_S3_DUAL_002_disposable_dual_routes_keep_events_and_resources_isolated(self) -> None:
        coding, firmware = self._dual_route_statuses()
        observed = reconcile(
            discover_suite(self.fixture.config), self._dual_process_snapshot(), self.fixture.config, now=NOW
        )
        lanes = {lane["lane_id"]: lane for lane in observed["lanes"]}
        self.assertEqual(["service:coding-example"], lanes["coding:operator-example"]["resources"])
        self.assertEqual(
        ...
        for signal_id, lane_id, workspace in (
            ...
            write_json(workspace / "manager-signals" / f"{signal_id}.json", {
                "schema": "manager-signal/v1", "signal_id": signal_id, "kind": "HELP",
                "created_utc": "2026-07-30T12:00:00Z", "lane_id": lane_id,
                "task": "operator journey", "phase": "synthetic", "summary": signal_id,
                "evidence_paths": [],
            })
        signals = {
        ...
        self.assertEqual("firmware:legacy-example", signals["firmware-signal"]["lane_id"])

    def test_S3_DUAL_003_docs_roles_cleanup_registry_templates_and_safeguard_are_bound(self) -> None:
        ...
        for check in ("ruff", "format", "basedpyright", "compileall", "orchestrator-tests", "watcher-tests", "attention-retention", "codex-integration", "synthetic-cleanup"):
            self.assertIn(check, safeguard)
        self.assertIn("worktrees/harness-candidate", safeguard)
