...
class ReconcileTests(unittest.TestCase):
    ...
    def test_named_current_attempt_collapses_unnamed_same_thread_without_conflict(
        ...
        old = self.fixture.status(
            label="boreal_old", doer="Boreal", task="D31", state="exited",
            board_tokens=["stm-a"], controller_pid=101, codex_pid=102,
        )
        old_value = json.loads(old.read_text(encoding="utf-8"))
        old_value.pop("doer")
        write_json(old, old_value)
        self.fixture.status(
            label="boreal_current", doer="Boreal", task="D31",
            board_tokens=["stm-a"], controller_pid=201, codex_pid=202,
            started=NOW + timedelta(seconds=1),
        )
        base = self.fixture.process_snapshot()
        info = type(base.processes[0])
        observed = self.observe(type(base)(True, (
    ...
    def test_same_named_doer_and_task_with_distinct_threads_remain_separate(self) -> None:
        first = self.fixture.status(
            label="boreal_one", doer="Boreal", task="D31",
            board_tokens=["stm-a"], controller_pid=101, codex_pid=102,
        )
        second = self.fixture.status(
            label="boreal_two", doer="Boreal", task="D31",
            board_tokens=["stm-a"], controller_pid=201, codex_pid=202,
            started=NOW + timedelta(seconds=1),
        )
        ...
        self.assertTrue(all(":Boreal:D31" in lane["lane_id"] for lane in observed["lanes"]))
        self.assertEqual("board:stm-a", observed["resource_conflicts"][0]["resource"])

    def test_current_named_attempt_wins_same_thread_historical_terminal_state(self) -> None:
        old = self.fixture.status(
            label="boreal_old", doer="Boreal", task="D31", state="exited",
            controller_pid=101, codex_pid=102,
        )
        old_value = json.loads(old.read_text(encoding="utf-8"))
        old_value.pop("doer")
        write_json(old, old_value)
        self.fixture.status(
            label="boreal_current", doer="Boreal", task="D31",
            controller_pid=201, codex_pid=202, started=NOW + timedelta(seconds=1),
        )
        base = self.fixture.process_snapshot()
        info = type(base.processes[0])
        observed = self.observe(type(base)(True, (
    ...
    def _manager_request(self, *, sidecar: bool = False) -> tuple[Path, dict[str, Any]]:
        ...
        value: dict[str, Any] = {
            ...
            "created_utc": NOW.isoformat().replace("+00:00", "Z"),
            "deadline_utc": (NOW + timedelta(minutes=10)).isoformat().replace("+00:00", "Z"),
            "declared_lane_id": "clean-f:Boreal:D31",
            "session_id": "thread-atlas",
            "live_lifetime": {"run_id": "manager-run-1", "process": {"pid": 101, "creation_time_utc": NOW.isoformat().replace("+00:00", "Z")}},
            "board": {"board_token": "STM-B", "board_id": "stm_b", "probe_uid": "probe-b", "vcom": "COM17"},
            "roots": {"project": ".agent-workspace/runtime/project"},
    ...
    def _manager_relay(self, request_path: Path, request: dict[str, Any], *, exact: bool = True) -> Path:
        ...
        write_json(
            relay,
            {
                ...
                "tool_argument_sha256": request["tool_argument_sha256"] if exact else "b" * 64,
                "server_snapshot": request["server_snapshot"],
                "exact_approved_call": {"tool": request["tool"], "arguments": request["arguments"]},
    ...
    def test_clean_f_manager_request_is_discovered_lane_correlated_and_removes_false_ambiguity(self) -> None:
        self.fixture.status(board_tokens=["STM-B"], declared_lane_id="clean-f:Boreal:D31")
        path, _ = self._manager_request(sidecar=True)
        observed = self.observe(self.fixture.process_snapshot())
        self.assertEqual([str(path)], [item["path"] for item in observed["requests"]])
        self.assertEqual("clean-f:Boreal:D31", observed["requests"][0]["declared_lane_id"])
        self.assertEqual("RELAY_READY", observed["requests"][0]["operational_state"])
        self.assertNotIn("active hardware lane has no current request identity for probe/serial/root audit", observed["lanes"][0]["resource_ambiguity"])

    def test_manager_relay_without_sidecar_is_observed_and_exact_binding_relays(self) -> None:
        ...
        observed = self.observe(self.fixture.process_snapshot())
        self.assertEqual([str(relay)], [str(item.path) for item in discover_suite(self.fixture.config)[0].relays])
        self.assertEqual("RELAYED", observed["requests"][0]["operational_state"])
        self.assertFalse(any(item["code"] == "RELAY_READ_ERROR" for item in observed["observation_errors"]))

    def test_manager_relay_without_sidecar_still_requires_exact_content_binding(self) -> None:
        self.fixture.status(declared_lane_id="clean-f:Boreal:D31")
        path, request = self._manager_request()
    ...
    def test_expired_exact_manager_relay_is_retained_but_not_actionable(self) -> None:
        ...
        value["expires_utc"] = (NOW - timedelta(seconds=1)).isoformat().replace("+00:00", "Z")
    ...
    def test_suite_lifetime_binding_proves_only_exact_live_server_pid_and_creation(self) -> None:
        self.fixture.status(declared_lane_id="clean-f:Boreal:D31")
        path, request = self._manager_request()
        request.pop("live_lifetime")
        request["mcp_server"] = {"name": "clean-k-mcp"}
        request["lifetime_binding"] = {
            "server_name": "clean-k-mcp", "pid": 101,
    ...
    def test_invalid_manager_request_sidecar_is_an_observation_error(self) -> None:
        ...
        self.assertTrue(any(item["code"] == "REQUEST_READ_ERROR" for item in observed["observation_errors"]))

    def test_invalid_present_manager_relay_sidecar_is_an_observation_error(self) -> None:
        self.fixture.status(declared_lane_id="clean-f:Boreal:D31")
        path, request = self._manager_request()
        relay = self._manager_relay(path, request)
        relay.with_name(relay.name + ".sha256").write_text("0" * 64 + "  " + relay.name + "\n", encoding="ascii")
        observed = self.observe(self.fixture.process_snapshot())
        self.assertEqual("RELAY_READY", observed["requests"][0]["operational_state"])
        self.assertTrue(any(item["code"] == "RELAY_READ_ERROR" for item in observed["observation_errors"]))
    ...
    def test_unscoped_request_is_not_lane_owned(self) -> None:
        ...
        self.assertIsNone(observed["requests"][0]["producer_identities"].get("session_id"))
    ...
    def test_two_owned_relay_requests_transition_without_watcher_writes(self) -> None:
        ...
        first = {
            ...
            "live_lifetime": {
                "run_id": "first-run",
                "process": {"pid": 101, "started_utc": NOW.isoformat().replace("+00:00", "Z")},
            },
        ...
        second = {
            ...
            "live_lifetime": {
                "run_id": "second-run",
                "process": {"pid": 201, "started_utc": NOW.isoformat().replace("+00:00", "Z")},
            },
        ...
        snapshot = type(base)(
            ...
            + (info(201, 1, "python", "controller", NOW), info(202, 201, "codex", "codex", NOW)),
            (),
            "fake",
        )
        ready = self.observe(snapshot)
        self.assertEqual({"RELAY_READY"}, {item["operational_state"] for item in ready["requests"]})
        ...
        self.assertEqual({"RELAYED"}, {item["operational_state"] for item in relayed["requests"]})
        self.assertEqual(request_bytes, {path: path.read_bytes() for path in request_bytes})
    ...
    def test_missing_creation_unknown_request_is_actionable_only_before_deadline(self) -> None:
        self.fixture.status()
        path, value = self._request(include_start=False)
        value["deadline_utc"] = (NOW + timedelta(minutes=1)).isoformat().replace("+00:00", "Z")
        write_json(path, value)
    ...
    def test_object_shaped_mcp_declaration_without_mcp_lifetime_never_relays(self) -> None:
        self.fixture.status()
        path, value = self._request()
    ...
    def test_creation_time_utc_proves_live_mcp_launcher_server_provider_lifetime(self) -> None:
        self.fixture.status()
        path, value = self._request()
        ...
        for name, timestamp, include_processes in (
            ...
            with self.subTest(name=name):
                ...
                snapshot = type(base)(
                    True,
                    tuple(base.processes)
                    + ((info(301, 1, "python", "mcp server", NOW),) if include_processes else ()),
                    (),
                    "fake",
                )
    ...
    def test_a26_closed_before_board_action_stays_exited_when_nested_parent_pid_is_reused(self) -> None:
        record = {
    ...
    def test_windows_creation_time_raw_milliseconds_are_usable_for_live_mcp_identity(self) -> None:
        record = {
        ...
        observed = self._mcp_observation(record, self._snapshot_with_process(195828, NOW))
    ...
    def test_a24_creation_utc_matching_live_pid_remains_running(self) -> None:
        ...
        observed = self._mcp_observation(record, self._snapshot_with_process(194892, NOW))

        self.assertEqual("MCP_RUNNING", observed["operational_state"])

    def test_nonterminal_mcp_pid_without_creation_evidence_remains_unknown(self) -> None:
    ...
    def test_partial_creation_unknown_request_is_actionable_only_before_deadline(self) -> None:
        self.fixture.status()
        path, value = self._request()
        ...
        self.assertIsNotNone(select_actionable(
            conditions_from_snapshot(current), current, observed_at=NOW,
            acknowledged_event_ids=set(),
        ))
    ...
    def test_terminal_checkpoint_before_mcp_creation_has_no_invented_mcp_or_ambiguity(self) -> None:
        self.fixture.status(state="exited", mcp_servers=["byo-firmware-stm-a"])
        (self.fixture.workspace() / "PARALLEL_CHECKPOINT.md").write_text("# complete\n", encoding="utf-8")
        snapshot = self.fixture.process_snapshot()
        observed = self.observe(type(snapshot)(True, (), (), "fake"))
        lane = observed["lanes"][0]
        self.assertEqual("CHECKPOINTED", lane["operational_state"])
    ...
    def test_running_lane_with_missing_declared_mcp_still_synthesizes_unknown(self) -> None:
        self.fixture.status(mcp_servers=["byo-firmware-stm-a"])
        observed = self.observe(self.fixture.process_snapshot())
        lane = observed["lanes"][0]
        self.assertEqual("RUNNING_CODEX", lane["operational_state"])
        self.assertIn("declared MCP server has no lane/session-correlated PID lifetime evidence", lane["resource_ambiguity"])
        self.assertEqual("MCP_STATE_UNKNOWN", observed["mcps"][0]["operational_state"])

    def test_waiting_relay_lane_with_missing_declared_mcp_still_synthesizes_unknown(self) -> None:
        self.fixture.status(mcp_servers=["byo-firmware-stm-a"])
        self._request()
        observed = self.observe(self.fixture.process_snapshot())
        lane = observed["lanes"][0]
    ...
    def test_process_unknown_lane_with_missing_declared_mcp_still_synthesizes_unknown(self) -> None:
        self.fixture.status(mcp_servers=["byo-firmware-stm-a"])
        observed = self.observe(self.fixture.process_snapshot(complete=False))
        lane = observed["lanes"][0]
    ...
    def test_unknown_lane_and_helper_running_are_active_mcp_expectations(self) -> None:
        ...
        self.assertIn("declared MCP server has no lane/session-correlated PID lifetime evidence", lane["resource_ambiguity"])
        self.assertEqual("MCP_STATE_UNKNOWN", observed["mcps"][0]["operational_state"])
        self.assertTrue(_lane_is_active_or_unknown({"operational_state": "HELPER_RUNNING"}))

    def test_explicit_terminal_mcp_lifetime_releases_exited_checkpointed_lane(self) -> None:
        self.fixture.status(
            state="exited", mcp_servers=["byo-firmware-stm-a"],
            declared_lane_id="canary:atlas:A22:B14",
        )
        (self.fixture.workspace() / "PARALLEL_CHECKPOINT.md").write_text("# checkpoint\n", encoding="utf-8")
        write_json(
    ...
    def test_terminal_lane_ignores_uncorrelated_mcp_lifetime_without_synthesizing_unknown(self) -> None:
        self.fixture.status(
            state="exited", mcp_servers=["byo-firmware-stm-a"],
            declared_lane_id="canary:atlas:A22:B14",
        )
    ...
    def test_terminal_lane_with_correlated_unknown_mcp_remains_nonreleasable(self) -> None:
        self.fixture.status(
            state="exited", mcp_servers=["byo-firmware-stm-a"],
            declared_lane_id="canary:atlas:A22:B14",
        )
        ...
        base = self.fixture.process_snapshot()
        info = type(base.processes[0])
        observed = self.observe(type(base)(True, (info(301, 1, "mcp", "synthetic mcp", NOW),), (), "fake"))
        lane = observed["lanes"][0]
        self.assertFalse(lane["resource_release_possible"])
        self.assertEqual("MCP_STATE_UNKNOWN", observed["mcps"][0]["operational_state"])
