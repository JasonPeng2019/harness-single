...
class FirmwareLifecycleCompatibilityTests(unittest.TestCase):
    ...
    def observe(self, snapshot):
        return reconcile(discover_suite(self.fixture.config), snapshot, self.fixture.config, now=NOW)
    ...
    def test_S1_LIFE_001_cross_route_contention_is_live_only(self) -> None:
        ...
        self.assertEqual(
            [{"resource": "board:stm-a", "owners": ["coding:worker-1", "firmware:relay-1"]}],
        ...
        exited = self.observe(type(self.fixture.process_snapshot())(True, (), (), "fake"))
    ...
    def test_S1_LIFE_002_relay_uses_explicit_lane_over_shared_thread_and_expires(self) -> None:
        coding = self.fixture.status(
            run="coding", label="coding", doer="Coding", task="C01",
            declared_lane_id="coding:worker-1",
        )
        ...
        firmware = self.fixture.status(
            run="firmware", label="firmware", doer="Firmware", task="F01",
            controller_pid=201, codex_pid=202, declared_lane_id="firmware:relay-1",
        )
        ...
        request = {
            ...
            "live_lifetime": {"run_id": "firmware-run-1", "process": {"pid": 201, "creation_time_utc": NOW.isoformat().replace("+00:00", "Z")}},
            "board": {"board_token": "STM-A", "board_id": "stm_a", "probe_uid": "probe-a", "vcom": "COM1"},
        ...
        request_path = self.fixture.workspace("firmware") / "manager-requests" / "firmware-relay-1.json"
        write_json(request_path, request)
        relay_path = self.fixture.workspace("firmware") / "manager-relays" / "firmware-relay-1.json"
        write_json(relay_path, {
            "schema": "suite-manager-relay/v1", "decision": "approved",
        ...
        relay["expires_utc"] = (NOW - timedelta(seconds=1)).isoformat().replace("+00:00", "Z")
